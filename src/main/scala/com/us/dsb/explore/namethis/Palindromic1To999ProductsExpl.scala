package com.us.dsb.explore.namethis
...
object Palindromic1To999ProductsExpl {
  ...
  def doIt0(): (Int, Int) = {
    ...
    val ijp =
      range.flatMap(i => range.map(j => (i, j, i * j)))
        .filter(ijp => {iterations += 1; isPalindromic(ijp._3)})
        .maxBy(ijp => ijp._3)
  ...
  def doIt7(): Option[(Int, Int)] = {
    ...
    for (diagNum <- 1 to MAX_FACTOR) {
      ...
      for (dummy <- 1 to 1;
           diagOffset = diagNum - 1
           ;//if {println(s"  - diagOffset := $diagOffset"); true};
           ...
           iRange = (MAX_FACTOR - diagOffset) to MAX_FACTOR
           ;//if {println(s"  - iRange := $iRange"); true};
           i <- iRange;
